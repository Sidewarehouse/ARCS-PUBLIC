...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	// 制御用変数宣言
	static double Fx = 0; // [N] X方向の力
	static double Fy = 0; // [N] Y方向の力
	static double Fz = 0; // [N] Z方向の力
	static double Mx = 0; // [Nm] X軸回りのトルク
	static double My = 0; // [Nm] Y軸回りのトルク
	static double Mz = 0; // [Nm] Z軸回りのトルク
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// ここに制御アルゴリズムを記述する
		Interface.Get6axisForce(Fx, Fy, Fz, Mx, My, Mz);
		
		Interface.SetCurrent(CurrentRef);	// [A] 電流指令の出力
		Screen.SetVarIndicator(Fx, Fy, Fz, Mx, My, Mz, 0, 0, 0, 0);	// 任意変数インジケータ(変数0, ..., 変数9)
