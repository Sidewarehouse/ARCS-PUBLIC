...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	constexpr std::array<TwoInertiaParamDef, N> PlantParamsArray = {
		PlantParams, PlantParams, PlantParams, PlantParams, PlantParams, PlantParams
	};	// 2慣性系パラメータ構造体の配列
	...
	if(CmdFlag == CTRL_LOOP){
		...
		// 6軸分の2慣性系シミュレータの例(ただし全軸独立で干渉は無し)
		iq[1] = StepWave(0.1, t);
		iq[2] = StepWave(0.2, t);
		iq[3] = StepWave(0.3, t);
		iq[4] = StepWave(0.4, t);
		...
		Screen.SetVarIndicator(ths[1], ths[2], ths[3], ths[4], ths[5], ths[6], 0, 0, 0, 0);	// 任意変数インジケータ(変数0, ..., 変数9)
		...
		Memory.SetData(Tact, t, ths[1], ths[2], ths[3], ths[4], ths[5], ths[6], 0, 0, 0);		// CSVデータ保存変数 (周期, A列, B列, ..., J列)
		// リアルタイム制御ここまで
	}
