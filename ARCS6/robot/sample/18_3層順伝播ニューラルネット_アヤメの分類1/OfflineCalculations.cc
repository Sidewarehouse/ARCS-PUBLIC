...
int main(void){
	...
	constexpr size_t OutSize = 1;		// 出力層のサイズ 
	...
	constexpr ActvFunc OutFunc = ActvFunc::IDENTITY;				// 出力層の活性化関数
	...
	constexpr NnDropout DropoutEna = NnDropout::ENABLE;				// ドロップアウトするかどうか
	constexpr NnShuffle EpochShfl = NnShuffle::ENABLE;				// エポック毎にデータセットをシャッフルするかどうか
	...
	FeedforwardNeuralNet3<
		IrisDatasets<MinbatSize>,	// データセットクラス名
		InSize,			// 入力層のサイズ
		HidSize,		// 隠れ層のサイズ
		OutSize,		// 出力層のサイズ 
		MinbatSize,		// ミニバッチサイズ
	...
	IrisDatasets<MinbatSize> IrisData;			// アヤメデータセットの生成(分類番号版)
