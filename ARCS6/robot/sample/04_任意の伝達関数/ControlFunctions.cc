...
bool ControlFunctions::ControlFunction1(double t, double Tact, double Tcmp){
	...
	const Matrix<1,3> N = {5,6,7};	// 伝達関数の分子の係数ベクトル (5*s^2 + 6*s + 7)
	const Matrix<1,4> D = {1,2,3,4};// 伝達関数の分母の係数ベクトル (s^3 + 2*s^2 + 3*s + 4)
	const double w = 30;					// [rad/s] 2次遅れ系の周波数
	const double z = 0.2;					// [-] 2次遅れ系の制動係数
	const Matrix<1,1> B = {w*w};			// 2次遅れ系の分子係数ベクトル (ω^2)
	const Matrix<1,3> A = {1, 2.0*z*w, w*w};// 2次遅れ系の分母係数ベクトル (s^2 + 2ζωs + ω^2)
	
	// 制御用変数宣言
	static double u = 0, y = 0; 				// 伝達関数の入力信号と出力信号
	static TransferFunction<2,3> C(N, D, Ts);	// 2/3次の伝達関数の生成
	static TransferFunction<0,2> G(B, A, Ts);	// 0/2次の伝達関数の生成(2次標準形)
